...
void UART4_IRQHandler(void) {
	...
	if (__HAL_UART_GET_FLAG(&DAISY_RX1_UART,
			UART_FLAG_PE) || __HAL_UART_GET_FLAG(&DAISY_RX1_UART, UART_FLAG_ORE)
			|| __HAL_UART_GET_FLAG(&DAISY_RX1_UART, UART_FLAG_NE) || __HAL_UART_GET_FLAG(&DAISY_RX1_UART, UART_FLAG_FE)) {
		// 1. Clear the error flags (Added UART_CLEAR_PEF)
		__HAL_UART_CLEAR_IT(&DAISY_RX1_UART,
				UART_CLEAR_PEF | UART_CLEAR_OREF | UART_CLEAR_NEF | UART_CLEAR_FEF);
...
void UART5_IRQHandler(void) {
	...
	if (__HAL_UART_GET_FLAG(&DAISY_RX2_UART,
			UART_FLAG_PE) || __HAL_UART_GET_FLAG(&DAISY_RX2_UART, UART_FLAG_ORE)
			|| __HAL_UART_GET_FLAG(&DAISY_RX2_UART, UART_FLAG_NE) || __HAL_UART_GET_FLAG(&DAISY_RX2_UART, UART_FLAG_FE)) {
		// 1. Clear the error flags
		__HAL_UART_CLEAR_IT(&DAISY_RX2_UART,
				UART_CLEAR_PEF | UART_CLEAR_OREF | UART_CLEAR_NEF | UART_CLEAR_FEF);
...
{
    // Check for Parity, Overrun, Noise, or Frame errors
    if (__HAL_UART_GET_FLAG(&DAISY_RX1_UART, UART_FLAG_PE) || __HAL_UART_GET_FLAG(&DAISY_RX1_UART, UART_FLAG_ORE)
        || __HAL_UART_GET_FLAG(&DAISY_RX1_UART, UART_FLAG_NE) || __HAL_UART_GET_FLAG(&DAISY_RX1_UART, UART_FLAG_FE)) {
...
{
    // Check for Overrun, Noise, or Frame errors
    if (__HAL_UART_GET_FLAG(&DAISY_RX2_UART, UART_FLAG_PE) || __HAL_UART_GET_FLAG(&DAISY_RX2_UART, UART_FLAG_ORE)
        || __HAL_UART_GET_FLAG(&DAISY_RX2_UART, UART_FLAG_NE) || __HAL_UART_GET_FLAG(&DAISY_RX2_UART, UART_FLAG_FE)) {
...
static void MX_USART_UART_Init(UART_HandleTypeDef *huart, USART_TypeDef *handle) {
	...
	uint32_t max_baudrate = SYSCLK_FREQ_HZ / 9; // 20 Mbps
